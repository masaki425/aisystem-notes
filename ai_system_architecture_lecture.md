...
**資産の再利用性（ロックイン回避）** — Harnessの資産がClaude Code以外のツールでどこまで再利用できるか。9.3で整理したツール間の仕様の違いが、実際の移植でどの程度の障壁になるかを確認する。ドキュメント層はそのまま渡せるはずだ。実行環境層は9.3の比較表に沿って変換を行う。手順保証層は、Hooks機構の差異がどこで問題になるかを観察する。
...
資産の再利用性について——三層モデルの各層で、実際の移植コストはどの程度だったか。ドキュメント層のポータビリティ、実行環境層の標準化の効果、手順保証層の設計哲学の差異。
