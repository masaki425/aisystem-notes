...
次の第9章では、このHarnessを別のAIツールに移植する。ソフトウェア工学における移植テストの手法を借りて、Harnessの設計がモデルやツールの違いによってどのような影響を受けるかを観察する。
...
第9章では、Harnessの設計がどこまでモデルやツールに依存しているかを確認するために、移植テストを行う。Harness一式——proposal.md、spec.md、Worker定義、検証スクリプト、Skills、Rules、Hooks設定——を、別のモデル・ツールにそのまま渡して同じタスクを実行する。Harnessにとっての「プラットフォーム」は二つの要素から成る。一つは**モデル**（Claude、GPT、Gemini）——自然言語の指示を解釈し、概念の抽出やYAMLの生成を実行する側だ。もう一つは**ツール**（Claude Code、Codex CLI、Gemini CLI）——HooksやSkillsといった実行基盤を提供し、Harnessのファイル群を読み込んでモデルに渡す側だ。モデルを変えれば、同じspec.mdの同じ文章に対する解釈が変わる。ノードの粒度、エッジの選び方、seed listの活用方法——すべてがモデルの傾向に影響される。ツールを変えれば、Stop Hookが手順を強制できるか、Skillsがどう発動するか、Worker定義がどのパスから読まれるかが変わる。Harnessを固定してこの二つの変数を動かすことで、設計の環境依存性と、モデルごとの解釈の傾向や応答特性の差異を同時に観察できる。
...
5.2で果物の検索を例にとった。Pythonのコードなら `"リンゴ" in text` の結果は全処理系で同じだ。自然言語の「果物について言及している箇所を見つけて」は、AIによって結果が異なりうる——あるモデルは品種名まで拾い、別のモデルはカテゴリ名しか拾わない。この揺らぎがモデル間の移植でも現れる。移植テストの文脈で言えば、spec.mdに「各論文から主要な概念をノードとして抽出し、概念間の関係をエッジとして記述する」と書いたとき、「主要な概念」の粒度も「関係」の範囲もモデルによって異なりうる。
...
**感受性の差** — 同じ指示書の中で、どの部分にどれだけ強く反応するかがモデルごとに異なる。長い指示書の冒頭の制約に強く反応するモデルもあれば、末尾の具体例に強く反応するモデルもある。Rulesの書き順や強調の仕方がモデルによって異なる効果を持つ。いわゆる「プロンプトのベストプラクティス」がモデルごとに異なるのは、この感受性の差に起因する。spec.mdのseed listの記述位置を変えただけで、あるモデルでは遵守率が上がり、別のモデルでは変化しない——こうした差異は事前には予測できない。
...
9.2ではモデルを変えたとき何が変わるかを整理した。本節では、Harnessの構造を三つの層に分け、各層でツールを変えたとき何が変わるかを整理する。第5章と第6章の議論に基づく分類だ。
...
**第1層（ドキュメント層）** は、ツールの変更にはほとんど影響されない——どのツールからでもテキストファイルとしてそのまま読める。ファイル形式の変換は不要だ。しかし、モデルとの関係はより複雑だ。ドキュメント層の中身は一様ではない。proposal.mdはユーザーが自分の言葉で意図を記述したものであり、どのモデルに渡しても内容は同じだ。一方、spec.mdはClaude Codeの/setupがproposal.mdから生成したものであり、Claudeが解釈しやすい形で構造化されている。たとえば、指示の優先度を示す強調の置き方、条件分岐の記述パターン、命名規則の表現方法——これらにClaudeの解釈傾向に合わせたプロンプト表現が入り込んでいる可能性がある。9.2で述べた感受性の差がここに関わる。各モデルにはプロンプトのベストプラクティスが存在し、あるモデルで効果的な表現が別のモデルでは無視されたり、逆効果になったりする。spec.mdをそのまま別のモデルに渡したとき、Claude向けの表現がGPTやGeminiで同じ効果を持つかは、実際に試してみなければわからない。第1層は「渡すこと」は容易だが、「同じように効くこと」は保証されない。このプロンプト表現のモデル依存性は第2層のWorker定義やSkillsにも共通する——これらも/setupがClaude向けに生成した自然言語だ。
...
検証スクリプト（validate_phase.py）やマージスクリプト（merge.py）はPythonで書かれた決定論的なコードであり、モデルにもツールにも依存しない——どの環境でも同じ結果を返す。
...
以上をまとめると、三層の移植容易性は異なる。ドキュメント層はファイルとしてはそのまま渡せるが、モデルが生成したプロンプト表現の依存性がある。実行環境層は標準化の恩恵でほぼ変換不要になりつつある。手順保証層はツールの設計哲学に依存し、移植テストで最も複雑な観察対象になる。9.2の揺らぎの枠組み（傾向・癖・感受性）と本節の三層モデルの枠組みを組み合わせて、移植テストで観察される差異がモデル由来なのかツール由来なのかを切り分ける。あるWorkerがseed listを無視したとき、それは第2層の配置の問題（ツール側）なのか、解釈の癖（モデル側）なのか。この区別がなければ、対処の方向を誤る。

### 9.4 実践 — 移植テストの実行と結果

第8章で構築したHarness——proposal.md（v1.3）、spec.md、Worker定義3体、validate_phase.py、merge.py——を、一文字も変えずにCodex CLIとGemini CLIに渡す。同じ3本の論文を入力とし、同じ成功基準で検証する。Harnessの指示は自然言語で書かれているため、渡すこと自体に書き直しは要らない。ただし、「読める」ことと「同じように解釈する」ことは別の問題であり、9.2で述べた揺らぎがここで実際に現れるはずだ。

移植テストでは、以下の3つの観点で結果を整理する。

**機能等価性** — 同じHarnessを渡して、同じタスクが完了するか。第8章の成功基準（YAMLスキーマ準拠、DOI存在、論文間接続）をvalidate_phase.pyで検証し、全ツールで同じ基準をPASSするかを確認する。

**非機能比較** — 機能的にPASSしたとして、出力の質にどのような違いが現れるか。ノードの粒度、エッジの選び方、命名の傾向、seed listの活用度など、validate_phase.pyでは検出できない質的な差異を観察する。同じseed listを渡しても、モデルが違えば概念の切り方は変わる。9.2で述べた解釈の傾向がここに直接反映される。これらの差異は「良い・悪い」ではなく、モデルごとの特性として観察する。

**ロックイン回避** — Harnessの資産がClaude Code以外のツールでどこまで再利用できるか。9.3で整理したツール間の仕様の違いが、実際の移植でどの程度の障壁になるかを確認する。ドキュメント層はそのまま渡せるはずだ。実行環境層は9.3の比較表に沿って変換を行う。手順保証層は、Hooks機構の差異がどこで問題になるかを観察する。

**【実験結果をここに配置する】**

### 9.5 移植テストが示したこと

**【実験結果に基づいて記述する。以下は構成の骨格のみ】**

移植テストの3つの観点から得られた知見を総括する。

機能等価性について——同じHarnessで同じタスクが完了したか。完了した場合、validate_phase.pyの結果にどのような差が出たか。完了しなかった場合、どの時点で何が原因で止まったか。

非機能比較について——出力の質の違いが何を示しているか。ノード粒度、エッジ選択、命名傾向の差異は、9.2で述べた傾向・癖・感受性のどれに帰属するか。

ロックイン回避について——三層モデルの各層で、実際の移植コストはどの程度だったか。ドキュメント層のポータビリティ、実行環境層の標準化の効果、手順保証層の設計哲学の差異。
...
第8章で抽出した設計原理——反復サイクル、意図の文書化、構造化、外部永続化、satisficingによる完了判断——は、特定のツールや課題に依存しない。第9章の移植実験は、自然言語で書かれたHarnessのポータビリティと、モデル間の解釈の揺らぎ、そして決定論的制御のツール依存性を検証した。
