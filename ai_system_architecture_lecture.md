...
移植テストの題材として、第7章の段階4（最終構成）を使う。1本の論文から概念ネットワークを構築するタスクであり、三層すべてを含みながらファイル数が少なく、差異の原因を追いやすい。

第7章では、Claude Code上で段階1（CLAUDE.mdのみ）から始めて段階4（Rules + Skills + Hooks）まで機能を積み上げた。ここでは、その段階4の構成一式をそのままCodex CLIとGemini CLIに渡す。段階を踏み直すのではなく、完成した構成を一括で渡したとき、別のモデル・ツールで何が起きるかを観察する。

移植するファイルは以下の通りだ。

```
第1層（ドキュメント層）
  CLAUDE.md              — プロジェクト概要（6行）
  .claude/rules/structurize.md  — type 7種・relation 8種の許可リスト（30行）
  .claude/skills/structurize/SKILL.md — 5ステップの手順書（54行）

第2層（実行環境層）
  上記ファイルの配置パスがツール固有
  → Codex CLI: AGENTS.md + .agents/skills/ に変換
  → Gemini CLI: GEMINI.md + .gemini/skills/ に変換

第3層（手順保証層）
  .claude/settings.json  — Stop Hook設定（10行）
  scripts/validate_yaml.sh — 9項目の整合性チェック（80行）

入力
  input/science.adt2760.pdf — Gianni et al., Science 2026
```

ドキュメント層のファイル（CLAUDE.md、Rules、SKILL.md）は、内容を一文字も変えずに3つのモデルに渡す。Claude Code上では段階4でノード28個・エッジ32個・孤立ノード0件・type/relation全項目準拠という結果が得られた。同じファイルをGPTとGeminiが読んだとき、同じ水準に到達するか。

実行環境層の変換は9.3の比較表に沿って行う。CLAUDE.mdの内容をAGENTS.md（Codex）/ GEMINI.md（Gemini）に転記し、SkillsとRulesの配置パスを各ツールの規約に合わせる。内容は同一、配置が異なる。

手順保証層はツールごとに対応が分かれる。Claude Codeでは.claude/settings.jsonのStop Hookがvalidate_yaml.shを自動実行する。Gemini CLIではHooks移行コマンド（`gemini hooks migrate --from-claude`）で設定を変換できるか。Codex CLIではStop hookがexperimental段階にあるため、同じ保証が得られるか。第7章の段階3→4で起きた質的な変化——確率的制御から決定論的制御への移行——が、ツール間でどの程度再現されるかを観察する。

移植テストの結果は、以下の3つの観点で整理する。

**機能等価性** — 同じファイル構成で、同じタスクが完了するか。Claude Codeの段階4と同じ成功基準（YAMLスキーマ準拠、type/relation許可リスト準拠、孤立ノードなし）で判定する。validate_yaml.shは決定論的なbashスクリプトなので、どの環境でも同じ基準で動く。

**非機能比較** — 機能的に完了したとして、出力の質にどのような違いが現れるか。ノードの粒度（何を概念として抽出するか）、エッジの選択（どの関係を拾うか）、type/relationの分類判断（同じ許可リストを渡しても境界の引き方が異なるか）、SKILL.mdの手順遵守率（Step 1の概要把握をコメントに記録したか、Step 4の自己チェックを実行したか）。9.2で述べた傾向・癖・感受性のどれに帰属するかを切り分けながら観察する。

**資産の再利用性（ロックイン回避）** — 84行のドキュメント層ファイル（Rules 30行 + SKILL.md 54行）がそのまま効くか。実行環境層の配置変換はどの程度の手間か。手順保証層のHooksは移行できるか。9.3で整理したファクトが実地でどこまで当てはまるかを確認する。

実行結果を整理する。三ツールとも同じ論文（Gianni et al., Science 2026）を入力とし、同じvalidate_yaml.shで検証した。

| 指標 | Claude Code | Codex CLI | Gemini CLI |
|------|-------------|-----------|------------|
| validate_yaml.sh | PASS | PASS | PASS |
| ノード数 | 28 | 17 | 7 |
| エッジ数 | 32 | 19 | 6 |
| type種使用 | 7/7 | 7/7 | 6/7 |
| relation種使用 | 8/8 | 6/8 | 4/8 |
| 孤立ノード | 0 | 0 | 0 |
| SKILL.md Step 1（概要把握記録） | 遵守 | 遵守 | 遵守 |
| ファイル命名 | `science_adt2760.yaml` | `science.adt2760.yaml` | `science.adt2760.yaml` |

三ツールともvalidate_yaml.shをPASSしたが、出力の規模は大きく異なった。同じ論文、同じSKILL.md、同じRulesを渡して、ノード数に4倍の差がある。

**ノードの粒度** — 三ツールの粒度に明確な勾配が観察された。Claudeは理論的枠組み（アイゲンの誤り閾値、準種理論）、発見の中間段階（3つの独立モチーフ→QT51→QT45）、分子の物理的性質（トランス活性、位置特異性）まで展開した28ノード。Codexは論文の主要ストーリーラインに沿った17ノード——発見手法（de novo選択）、中心的機構（相補鎖合成・自己鎖合成）、障害と解決策（鎖阻害・ヘキサマー補助基質）。Geminiは論文の要旨レベルの7ノード——リボザイム、基質、自己複製、仮説、条件、忠実度、パラドックス。Geminiは自己複製の2つの半サイクル（相補鎖合成と自己鎖合成）を個別ノードとせず `rna_self_replication` 1ノードに集約し、実験手法（de novo選択）と技術的障害（鎖阻害・補助基質）を丸ごと省略した。

**エッジの構造** — ノード数の差はネットワーク構造の差として現れた。Claudeのネットワークはメッシュ型で、概念間に横の接続が多い。`derives` チェーン（three_motifs → qt51_ribozyme → qt45_ribozyme）で発見経路を表現し、`contains` で分子の性質を接続する。Codexはやや線形で、発見→機構→検証の流れに沿っている。Geminiはスター型で、6本のエッジのうち4本がqt45_ribozymeを起点とし、概念間の横接続がほぼない。

**relation の使用** — relation 8種のうち、Claudeは全種を使い、Codexは6種（containsとderivesが不使用）、Geminiは4種（causes、compares、contains、derivesが不使用）だった。Rulesに「これ以外のrelationは使わない」と書いた禁止方向の制約は三ツールとも完全に遵守した。しかし「使え」方向——8種をすべて活用せよ——は明示していなかったため、モデルの判断に委ねられた。Codexではsupportsが47%を占め、Geminiではcauses（因果関係）が一度も使われなかった。ネットワークに因果構造がない——「AがBを引き起こす」という関係が存在しない——のは、同じ論文を読んだ結果として注目に値する。

**命名の揺らぎ** — 三ツールでIDが完全一致したノードは `qt45_ribozyme` と `triplet_substrates` の2件のみ。概念的に対応するが命名が異なるペアが多数あった。Codexは機能的命名（`complementary_strand_synthesis`）、Claudeは生化学用語（`minus_strand_synthesis`）、Geminiは抽象的命名（`high_fidelity`）を好む。seed listがなければ命名は揃わない——これは第8章の設計判断（seed listの必要性）を裏付ける結果だ。

**SKILL.md の解釈** — Step 1（概要把握のYAMLコメント記録）は三ツールとも遵守した。Step 2（ノード候補の抽出）では差が出た。SKILL.mdには「以下の優先順で探す」として5段階のリスト（中心対象→直接の関連要素→比較対象→理論的枠組み→課題・障害）を記載した。Claudeは5段階すべてから網羅的に抽出した。Codexは3段階目（比較対象）まで。Geminiは1〜2段階目（中心対象・直接の関連要素）で停止し、それ以降からの抽出が極めて少なかった。「以下の優先順で探す」を「全部探す」と読むか「重要なものだけ探す」と読むか——9.2で述べた感受性の差がここに現れた。

**ファイル命名** — Rulesに「入力ファイル名から拡張子を除いたもの」と記載した。入力は `science.adt2760.pdf`。CodexとGeminiはルール通り `science.adt2760.yaml` を生成した。Claudeはドットをアンダースコアに変換して `science_adt2760.yaml` を出力した。Claudeだけがルールのリテラルな解釈から逸脱している——「ファイル名にドットが含まれると拡張子と紛らわしい」という判断を自発的に行ったと推測される。これは9.2の「癖」に該当する。

**metadata** — Rulesには「metadata, nodes, edges の3セクションを必ず含める」としか定義しなかった。Claudeは著者名・ジャーナル名・年・DOIを自発的に追加した。Geminiも著者名と年を追加した。Codexはtitle・source・descriptionのみ。書誌情報の追加は指示にないが、Rulesの要件（3セクションの存在）は三ツールとも満たしている。

**validate_yaml.sh の書式依存性** — 移植テスト中に、validate_yaml.sh自体の問題が発見された。このスクリプトはYAMLパーサを使わずgrepでテキストパターンを検索する実装だ。YAMLでは `  - source: node_a` と `  -\n    source: node_a` は等価だが、grepパターンは後者にしか一致しない。Claudeの出力は前者の書式を使っていたため、エッジのsource/targetが検出されず偽陽性の孤立ノードが報告された。決定論的制御として設計したスクリプトが、モデル間の出力書式の差異に対して脆弱だった。対策としては、`yq` やPythonのPyYAMLを使ったパース方式への書き換えが考えられる。

各ツールの実行に使ったファイル一式と出力結果は、[examples/chapter9/](https://github.com/masaki425/aisystem-notes/tree/main/examples/chapter9/) に保存されている。

> 概念ネットワーク可視化: [Claude Code（28ノード・メッシュ型）](https://masaki425.github.io/aisystem-notes/examples/chapter7/visualization.html) / [Codex CLI（17ノード・線形）](https://masaki425.github.io/aisystem-notes/examples/chapter9/codex/visualization.html) / [Gemini CLI（7ノード・スター型）](https://masaki425.github.io/aisystem-notes/examples/chapter9/gemini/visualization.html)

### 9.5 移植テストが示したこと

9.4の結果を、9.2の枠組み（傾向・癖・感受性）と9.3の三層モデルで整理する。

**機能等価性** — 三ツールとも同じタスクを完了し、validate_yaml.shをPASSした。「1本の論文から概念ネットワークを構築してYAMLに出力する」というタスクは、モデルとツールを変えても達成できた。ドキュメント層のファイル（SKILL.md、Rules）が自然言語で書かれているため、どのモデルでも「読める」。ただし、「読める」と「同じ水準で実行する」は異なる——ノード数に4倍の差があることが示すように、機能等価性とは「完了するかどうか」であって「同じ結果になるかどうか」ではない。

**非機能比較** — 出力の質の差異は、9.2で整理した3つの性質に帰属できた。

解釈の傾向（一貫した方向性）として、粒度の勾配が最も顕著だった。Claude（28ノード、網羅的）、Codex（17ノード、主要ストーリーライン）、Gemini（7ノード、要旨レベル）。この勾配はrelation種の使用数（8→6→4）とネットワーク構造（メッシュ→線形→スター）にも反映されている。モデルごとに「どの程度の詳細さで論文を構造化するか」の基準が異なり、それが一貫した出力の差として現れる。

解釈の癖（予測しにくい反応）として、Codexがcontains/derivesを使わなかったこと、Geminiがcausesを使わなかったこと、Claudeがファイル名のドットをアンダースコアに変換したことが該当する。いずれもRulesに明示的に違反しているわけではないが、同じ指示を読んで異なる反応が生じている。特にGeminiのcauses不使用は、ネットワーク全体から因果構造が消失するという構造的な帰結をもたらした。

感受性の差として、SKILL.md Step 2の「優先順リスト」への反応が異なった。Claudeは5段階すべてを探索対象と解釈し、Geminiは上位1〜2段階で停止した。同じ文章の「以下の優先順で探す」という指示に対する感受性の差が、ノード数の差の主因の一つだ。

**資産の再利用性** — 三層の移植コストを整理すると以下の通りだ。

ドキュメント層（SKILL.md 54行、Rules 30行）は**内容の書き換えゼロ**で三ツールに渡せた。自然言語で書かれた指示のポータビリティは実証された。ただし「渡せる」と「同じように効く」は別の問題であり、SKILL.md Step 2の解釈差（上述）はドキュメント層の限界を示している。
...
手順保証層は**ツール間で最も差が大きかった**。Claude CodeのStop Hookはvalidate_yaml.shの失敗時に実行をブロックし、修正を強制する。Gemini CLIのAfterAgent hookもほぼ同等の機能を提供した。Codex CLIのagent-stop hookはexperimental段階であり、今回はPASSしたため差が顕在化しなかったが、失敗時のブロック強制力は未検証のままだ。三ツールとも「スクリプトを実行して結果を報告する」ところまでは共通だが、「失敗したら確実にブロックする」ことを保証できるのはClaude CodeとGemini CLIの2ツールであり、Codexは発展途上にある。

**validate_yaml.sh自体の移植耐性** — 想定外の発見として、決定論的スクリプト自体がモデル間の出力書式の差異に対して脆弱だった。grepベースの実装はYAMLの等価な書式バリエーションを正しく処理できず、Claudeの出力で偽陽性の孤立ノード検出が発生した。また、ノード数の下限が「1個以上」としか定義されていなかったため、Geminiの7ノードでも全項目PASSした——形式的な整合性（well-formedness）は検証できるが、概念的な十分性（adequacy）は検証できない。これは第2章で述べた「決定論的制御で強制できるのは形式的な条件であり、意味的な判断は確率的制御に委ねるしかない」という原理の実例だ。対策として、grepをYAMLパーサに書き換えることで書式依存性は解消できる。十分性については、ノード数の下限値（例: 15以上）をスクリプトに追加する方法と、「何ノードが適切か」をproposal.mdで明示する方法がある。前者は汎用性を犠牲にし、後者はドキュメント層での対処——5.6の「困ったら入れる」の原則に沿ったアプローチだ。

### 9.6 実用的な指針

移植テストの経験に基づく実用的な指針を示す。

**ドキュメント層は最初からポータブルに設計する。** proposal.mdやspec.mdにツール固有の記法を混ぜない。自然言語で完結させる。これは移植のためだけでなく、人間が読んで理解できるためでもある。
...
**手順保証層はツールの設計哲学で対処する。** アプリケーションレベルのきめ細かいHooksが必要なプロジェクトでは、Claude CodeやGemini CLIが適している。OSレベルのサンドボックスで十分なプロジェクトでは、Codex CLIのアプローチも有効だ。各ツールのHooks機能は急速に進化しているため、移植時点での最新の対応状況を確認する。

**解釈の揺らぎはテストで対処する。** 9.2で述べたモデル間の傾向・癖・感受性の差異は、事前に完全には予測できない。まずそのまま渡して実行し、問題が出た箇所だけを修正する反復的なアプローチが現実的だ。
...
| 手順保証層移行 | — | 設定書き換え | `migrate --from-claude` |
...
第8章で抽出した設計原理——反復サイクル、意図の文書化、構造化、外部永続化、satisficingによる完了判断——は、特定のツールや課題に依存しない。第9章の移植テストは、自然言語で書かれたHarnessのポータビリティと、モデル間の解釈の揺らぎ、そして決定論的制御のツール依存性を観察した。
