...
週次レポートがうまく回り始めたところで、上司から「朝礼のプレゼン資料も作ってほしい。それと、来月のチーム懇親会の企画も進めてくれ」と言われたとする。週次レポート、プレゼン資料、懇親会の企画——この3つを一つのセッションで順番にこなすと、プレゼン資料を作っている間にレポートの文脈が薄れ、懇親会の企画に入る頃にはコンテキストが逼迫する。

Agentsを使って、Leadが統括しWorkerがそれぞれのタスクを担当する構成にできる。

```
Lead: タスクを分配し、全体の整合性を確認
  ├── Worker A: 週次レポート作成（データ収集→分析→執筆）
  ├── Worker B: プレゼン資料作成（レポートの要点をスライド化）
  └── Worker C: 懇親会企画（会場・食事・スケジュール調整）
Lead: Worker BはWorker Aの完了待ち、Worker Cは並行で実行
```

**何が変わるか**: 各Workerが自分のタスクだけに集中できる。Worker C（懇親会）はWorker Aの完了を待たずに並行で進められる。Leadは「懇親会の予算が部署全体の予算枠を超えていないか」のような全体の整合性だけを確認する。

**何がまだ変わらないか**: Worker間の文脈共有にはファイルI/Oが必要で、オーバーヘッドがある（Worker BはWorker Aの出力ファイルを読む必要がある）。タスクが少なければAgentsなしの方がシンプルで効率的だ。
