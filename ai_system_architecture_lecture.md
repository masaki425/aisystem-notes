...
**確率的制御と決定論的制御の相互作用** — 9.4の結果は、第2章で述べた二つの制御の関係について新たな知見をもたらした。

確率的制御（Rules、Skills）は、モデルの傾向に応じて異なる方向に作用した。Claudeに対しては探索のガイド、Codexに対してはデフォルトの確認、Geminiに対しては制限と教育の二方向——同じファイルが、モデルとの関係によって異なる機能を持つ。これは「自然言語の仕様書はモデルの事前知識とどう相互作用するか」という問いであり、9.2で述べた「感受性の差」の具体的なメカニズムを示している。

決定論的制御（Hooks）は、より驚くべきパターンを示した。validate_yaml.shという同一のスクリプトが、Claudeには品質向上の刺激として、Codex/Geminiには行動の委縮として作用した。第2章では確率的制御と決定論的制御を「どちらをどこに配置するか」の設計判断として論じたが、移植テストは**同じ決定論的制御がモデルの傾向によって逆方向に作用しうる**ことを示した。Hooksの設計は、スクリプトの論理だけでなく、それを受け取るモデルの傾向も考慮する必要がある。
