...
段階4の出力YAMLを可視化すると、7.1で述べた「マインドマップの機械可読版」が実際にどのような図になるかが見える。28個のノードが7色（typeごと）で色分けされ、32本のエッジが関係の種類ごとに色と線種で区別される。中心にはQT45リボザイムが最も多くの接続を持つハブとして位置し、そこからRNA自己複製、RNAワールド仮説、鎖阻害問題といった概念へ放射状にエッジが伸びている。CLAUDE.md 6行から始めて、4段階を経てこのネットワークが得られた。[インタラクティブ版を見る](https://masaki425.github.io/aisystem-notes/examples/chapter7/visualization.html)
...
そして、本章ではMCPとAgentsを使わなかった。論文1本の構造化にはこの4ステップで十分だからだ。MCPとAgentsが必要になるのは、複数の論文を横断的に比較分析するとき——つまり、タスクの複雑さが増したときだ。タスクが複雑になると、「機能を足す」だけでは対処できない。プロジェクト全体を通じてHarnessをどう設計し運用するかの原理が必要になる。第8章では、研究にも開発にも共通する「開発サイクル」を軸に、その原理を整理する。
...
## 第8章 設計原理 — 開発サイクルとHarness設計

第7章では、論文1本という限定された課題に対して、機能を一つずつ追加する体験をした。CLAUDE.md 6行で始めて、Rules、Skills、Hooksと段階的に馬具を装着していった。

しかし実際のプロジェクトでは、タスクはもっと複雑だ。入力データが増え、処理が多段階になり、複数セッションにまたがり、途中で仕様が変わる。第7章のように「機能を足す」だけでは対処できない。必要なのは、プロジェクト全体を通じてHarnessをどう設計し運用するかの原理だ。

本章では、研究にも開発にも共通する「開発サイクル」を軸に、Harness設計の一般原理を整理する。

### 8.1 開発サイクル — 自由度の配置がHarness設計の核心

研究であれ開発であれ、プロジェクトは一方向に進むのではなく、サイクルを描く。
...
つまり、「自由度が最大に戻る」のではなく、**問題の粒度に応じた自由度に戻る**。これがサイクルの核心だ。
...
第7章では、CLAUDE.md 6行でプロジェクトを開始できた。論文1本の構造化なら、それで十分だった。

しかし、タスクが複雑になるとCLAUDE.mdだけでは足りなくなる。論文が3本に増えたら？チャンクに分割して並列処理する必要が出たら？どの基準でエッジを評価するか、AIに任せる範囲はどこまでか——こうした判断が増えてくると、CLAUDE.md、Rules、Skills、Hooks、Agents定義のすべてを「何に基づいて」書くのかが問題になる。

答えは、サイクルの起点として**プロジェクトの意図を一つの文書にまとめること**だ。何を作りたいのか（目的）、何をもって成功とするのか（基準）、どういう方針で進めるのか（制約）、そしてAIに委ねてよい範囲（自由度の配置）。この文書が、Harnessのすべてのファイルを生成するための「設計図の設計図」になる。
...
特に重要なのは自由度の配置を明示することだ。「AIが自由に判断してよい範囲」を文書の中で宣言する。ノードIDの命名規則をAIに任せるなら、そう書く。書かなかった項目はAIが勝手に変えるべきではない——この暗黙のルールを文書化することで、確率的制御の精度が上がる。

意図の文書がサイクルの起点である理由は、もう一つある。8.1で述べた「どこに戻るか」の判断で、**戻り先の基準になる**のがこの文書だからだ。評価段階で「何かがおかしい」と感じたとき、意図の文書と現状を照合すれば、問題が意図レベルなのか、仕様レベルなのか、実装レベルなのかを判定できる。この文書がなければ、「元々何がしたかったか」がセッションをまたいで失われ、修正のたびに方向がぶれる。

### 8.3 構造化 — 意図から仕様への変換

8.1のサイクル図で、意図の表明と実装の間に「構造化」がある。提案書を仕様書に変換するステップだ。ここで何が起きているかを理解することは、Harness設計にとって重要だ。
...
この変換で重要なのは、**何をAIの解釈に委ね、何を人間が決めるか**が明確になることだ。提案書に「Claude判断可」と書かれた項目——たとえばノードIDの命名規則やPhase内のタスク順序——はAIが決定する。それ以外の項目——成功基準、スキーマ定義、品質の優先順位——は提案書の記述がそのまま仕様書に転写される。
...
8.1のサイクルで「仕様の修正に戻る」とは、この構造化ステップに戻ることを意味する。タスク分解を変える、Phase構成を組み替える、検証基準を修正する——いずれも仕様書レベルの変更であり、提案書（意図）は変わらない。逆に「提案書の見直しに戻る」は、意図そのものが変わる場合だ。この区別ができるのは、構造化ステップで意図と設計が分離されているからだ。
...
第1章で述べたステートレス性を思い出してほしい。セッションが終わるとモデルの側には何も残らない。翌日の続きは、外部ファイルに書き出された情報だけが手がかりになる。

開発サイクルを維持するために必要な外部ファイルは、役割で整理できる。
...
AIを使った作業には、「いつ終わりにするか」という本質的な問題がある。評価と修正のループは理論上は無限に続けられる。AIに「もっと良くして」と言えば、AIは何かを変更してくれる。変更が本当に改善かどうかは別として。
...
**なぜ収束しないか**: AIによるレビュー→修正ループが収束しない理由は構造的だ。各レビューで「改善点」が見つかるが、修正すると別の箇所に影響が出る。新たなレビューで別の「改善点」が見つかる。各ステップは確率的に正しいが、全体としては振動する——これは8.1の「どこに戻るか」の判断が、毎回異なるレビュー結果に基づいて異なる場所に戻るために起きる。

**閾値の設計**: 意図の文書（提案書）に完了基準として組み込むのが自然だ。サイクルの起点で「ここまでできたら終わり」を宣言しておく。そして、その遵守をStop Hookに組み込む。「レビュー回数が3回を超えたら人間に判断を委ねる」「致命的バグが0かつ重要バグが2以下なら完了を許可」のような条件をスクリプトで実装する。閾値自体は人間が設計し（確率的制御では定義できない判断）、遵守はHooksが保証する（決定論的制御）。

satisficingは8.1のサイクルと組み合わせて初めて機能する。サイクルがなければ「いつ終わるか」の問題自体が生じない。サイクルがあってもsatisficingがなければ、ループが永遠に続く。両者はセットだ。

> **実用上のポイント**: 提案書は最初から完璧である必要はない。対話を通じてある程度まとまったらサイクルを回し、レビューで問題を発見し、再び対話に戻って提案書を詰め直す。サイクルを回すことで、最初には見えなかったゴールが明確になっていく。このプロセス自体が本章の原理——開発サイクル、構造化による意図と設計の分離、外部永続化によるサイクルの維持、satisficingによる出口の判断——を体現している。全てを事前に決める必要はない。このサイクルは、回しながら明確にしていける構造になっている。

### 8.6 適用例 — タスクの性質で設計判断はどう変わるか

ここまでの原理は抽象的だ。「開発サイクル」や「自由度の配置」と言われても、自分のタスクにどう適用するかが見えないかもしれない。本節では、性質の異なる3つのタスクを例に、同じ原理がタスクによってどう適用されるかを示す。
...
**サイクルの特徴**: 概念分析は解釈の揺れが大きいタスクだ。同じテキストを2回処理すると異なるノードが抽出されうる。評価段階で「この概念が抜けている」「この関連は的外れだ」と気づいたとき、戻り先は実装ではなく仕様——「どの粒度でノードを定義するか」の判断基準——であることが多い。中間成果物（各チャンクの抽出結果）を逐次ファイルに書き出すことが特に重要になる。auto-compactで前のチャンクの文脈が消えても、ファイルに残っていれば一貫性を保てる。

**satisficingの閾値**: 概念間の関連スコアが「正確かどうか」を客観的に検証する方法がない。したがって「全ノードにカテゴリが付与されている」「全エッジに5基準スコアが付いている」のような形式的な完全性を閾値とし、スコアの妥当性は人間の評価に委ねる。
...
**サイクルの特徴**: 画像が大量にある場合、Agentsによるタスク分割が有効だ。画像群を条件別・撮影バッチ別にグループ分けし、各Workerが独立して前処理・アルゴリズム適用・物理量算出を実行する。Leadが全Workerの結果を統合し、条件間の統計比較を行う。評価段階で「値が桁違いにおかしい」と気づいたとき、戻り先は提案書（キャリブレーション定数の見直し）か仕様（アルゴリズムの選択方針の変更）か実装（パラメータ調整）かが明確に分かれる。
...
**サイクルの特徴**: コード実装ではgit commitがサイクルの「チェックポイント」として特に重要だ。Phase毎にcommitすることで、問題が発生したときに前の状態に戻せる。テストが失敗したとき、戻り先は明確だ——テストコードと仕様書を照合すれば、問題が実装のバグなのか、仕様の矛盾なのかが分かる。第9章の移植実験で確認されたように、Hooks無しではcommitがスキップされる傾向が強い。シミュレーション実装こそ、Hooksによるcommit強制が最も価値を発揮するタスクだ。
...
### 8.7 まとめ — 新しいプロジェクトで原理をどう使うか

本章の原理は「最初に全部決めてから走れ」と言っているのではない。サイクルを回しながら明確にしていく構造を持て、と言っている。

**対話から始める。** 最初にあるのは曖昧な意図だ。「こういうことがしたい」を対話で言語化し、提案書にまとめる。この段階で完璧を求めない。自由度の配置も失敗のコストの見極めも、やってみなければ分からない部分がある。ある程度まとまったら、次に進む。

**サイクルを回して発見する。** 提案書を構造化し、仕様書を生成し、実装を走らせ、評価する。最初のサイクルで見えてくるのは、提案書に書かなかったこと——暗黙の前提、見落としていた制約、AIに任せるべきでなかった判断——だ。この発見こそがサイクルの価値であり、最初から全部見通せるなら、そもそもサイクルは要らない。

**対話に戻って提案書を詰める。** 発見した問題をもとに、再び対話する。「ここはAIに任せてよかったのか」「この完了基準は適切か」「Phase構成を変えるべきか」。対話で考えを整理し、提案書を更新し、再びサイクルを回す。

この「対話→提案書→サイクル→レビュー→対話」の反復が、本章の5つの原理を体現している。

- **開発サイクル（8.1）**がこの反復の骨格を与える
- **意図の文書化（8.2）**が反復のたびに精度を上げる起点になる
- **構造化（8.3）**が意図と設計を分離し、「どこに戻るべきか」を判断可能にする
- **外部永続化（8.4）**が反復をセッションをまたいで維持する
- **Satisficing（8.5）**が「この反復はここで止める」の判断基準を提供する

タスクの内容は研究と開発で違っても、この反復の構造は同じだ。次の第9章では、この原理に基づくHarnessを別のAIツールに移植したとき、何がそのまま動き何を書き換える必要があるかを、実験結果に基づいて述べる。
