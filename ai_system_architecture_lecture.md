...
そして、本章ではMCPとAgentsを使わなかった。論文1本の構造化にはこの4ステップで十分だからだ。MCPとAgentsが必要になるのは、複数の論文を横断的に比較分析するとき——つまり、タスクの複雑さが増したときだ。第8章では論文を3本に拡張し、そこで初めてMCP、Agents、そしてproposal.mdの必要性が生まれる過程を体験する。
...
第7章の段階1から段階4への進行は、AIの自由度が段階的に制約されていくプロセスだった。これをプロジェクトの時間軸に沿って展開すると、より一般的なパターンが見えてくる。
...
第7章の段階1〜3で、CLAUDE.md → Rules → Skills と段階的に制約を追加した。しかし実際のプロジェクトでは、これらのファイルを「何に基づいて」書くのかが問題になる。
