...
### 8.7 まとめ — 原理と実践はどう対応したか

8.6の4サイクルを、「5つの原理の連動」の表に照らして振り返る。

サイクル1で最初に問題になったのは自由度の配置（8.1）だった。命名規則をWorkerの自由裁量に委ねたことで、同一概念に異なるIDが付与された。「概念の解釈」は任せてよい自由度だが、「IDの命名」は統制すべき自由度だった。この線引きは事前には見えず、サイクルを回して初めて明確になった。

この問題への対処が、意図の文書化（8.2）の精緻化を駆動した。提案書v1.1でB5（命名規則）を追加し、v1.2で収束条件を強化し、v1.3で検証方法を拡充した。提案書がv1.0からv1.3へと更新される過程そのものが、4サイクルで「何を学び、何を決めたか」の記録になっている。そして提案書を修正するたびに、/setupによる構造化（8.3）が機能した。提案書のB5にseed listを追加して/setupを再実行すると、仕様書とWorker定義3体に一貫して波及する。意図と設計が分離されていなければ、修正のたびに複数のファイルを手動で整合させる必要があり、サイクルを重ねるほど整合性が崩れていただろう。

サイクルがセッションをまたいで機能したのは、外部永続化（8.4）のおかげだ。Stop Hookがprogress.mdの更新とgit commitを強制し、復帰ポイントを確保した。issues.mdへの自動記録は、サイクル4でAUTO-001がLeadの修正行動を駆動した例が示すように、「問題の発見」と「対処」を紐づける外部記憶として機能した。

本稿ではサイクル4で区切ったが、実プロジェクトならここからまだサイクルは続く。孤立ノード率50%、共有ノード5個——改善の余地はいくらでもある。サイクル3で一度完了と判断しながら構造的問題に気づいてサイクル4に再入場したように、satisficing（8.5）は「ここで打ち止め」ではなく「今の時点で十分か」というサイクルごとの判断だ。4サイクルで止めたのはデモの制約であって、原理の帰結ではない。

これらを通じて見えるのは、**5つの原理は事前に設計するものではなく、サイクルを回す中で段階的に整えていくもの**だということだ。サイクル1では意図の文書化（提案書v1.0）だけで始め、命名の問題が見えてから自由度の配置を修正し（v1.1）、接続密度の低さが見えてから収束条件を強化し（v1.2）、数値検証の甘さが見えてからスクリプトを追加した（v1.3）。第7章でCLAUDE.md 6行から始めて4段階で馬具を足していったのと同じ原理が、ここではプロジェクト全体の設計レベルで機能している。
