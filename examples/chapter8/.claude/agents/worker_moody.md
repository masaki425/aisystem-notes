...
|----|-------|------|-----------------|
| `rna_world` | RNAワールド | concept | ✅ **必ず使用すること（v1.2で必須化）** |
| `luca` | LUCA（全生物最終共通祖先） | concept | ✅ 該当する場合は必ず使用 |
| `genetic_code` | 遺伝暗号 | concept | ✅ **必ず使用すること（v1.2で必須化）** |
| `in_vitro_selection` | in vitro選択 | method | 言及があれば使用 |
| `prebiotic_chemistry` | プレバイオティック化学 | concept | ✅ 該当する場合は必ず使用 |
| `rna_self_replication` | RNA自己複製 | process | 言及があれば使用 |

**⚠️ v1.2 必須化ルール**: サイクル2でMoody論文が `rna_world` を使用しなかったため、v1.2ではMoody論文に `rna_world` と `genetic_code` の使用を明示的に要求する。LUCAの研究はRNAワールドと遺伝暗号に必ず言及するため、これらのノードが欠落することはありえない。
