...
|-----|------|---------|
| 1.0 | 2026-03-13 | 初版作成 |
| 1.1 | 2026-03-13 | B5にノードID命名規則と共通ノードseed listを追加（サイクル1で命名の揺れによるマージ品質低下を確認したため） |
| 1.2 | 2026-03-13 | B3の収束条件を強化（論文間エッジ基準の引き上げ）、B5のseed list運用ルールを厳格化（サイクル2で統合ネットワークの接続密度が不十分と判明したため） |
| 1.3 | 2026-03-13 | D5の検証方法にB3条件3・4のスクリプト検証を追加（サイクル3でLeadの数値判定が甘かったため。定量的基準は決定論的制御で検証すべき） |
...
- サイクル2でMoody論文がrna_worldを使用しなかった（LUCAの論文がRNAワールドに言及しないのは不自然）。これは「言及があれば使用」という曖昧な指示が原因。v1.2ではMoodyにrna_world、genetic_codeの使用を明示的に要求する。
...
- **サイクル2の実行結果**: seed list導入で共有ノードが3個に増加（rna_world, in_vitro_selection, prebiotic_chemistry）。ただし統合ネットワーク60ノード中47ノード（78%）が他論文と未接続。Moody論文がrna_worldノードを使用せず。論文間エッジ11本は3論文の統合としては不十分。
- **サイクル3の実行結果**: seed list必須化で共有ノードが5個に増加。論文間エッジ17本。孤立ノード率は78%→70%に改善したがB3条件4（60%以下）は未達。ただしLeadは「全項目PASS」と報告——数値基準の確率的評価が甘かった。定量的基準はスクリプトで検証すべきと判断し、v1.3でvalidate_phase.pyに条件3・4を追加。
...
| ~~統合ネットワークの接続密度が低い~~ | ~~78%のノードが他論文と未接続~~ | **サイクル2で発生を確認 → B3の収束条件強化 + seed listの運用ルール厳格化で対処** |
| ~~Workerがseed listの概念をスキップする~~ | ~~論文間の共有ノードが不足~~ | **サイクル2でMoodyがrna_worldを不使用 → seed listの該当論文を明示的要求に変更** |
...
| **seed listの運用を「任意」から「必須」に変更** | **確率的制御（「言及があれば」）では不十分だった。明示的要求に切り替え** | **Hookで強制（seed listのID存在をスクリプトで検証）→ 過剰。仕様の明確化で十分と判断** |
| **収束条件に論文ペア間エッジ数を追加** | **「1本以上」では統合の品質を保証できなかった** | **固定数の論文間エッジをseed listに事前定義（→ 過剰な制約。Leadの判断に委ねる方が柔軟）** |
| **定量的基準をスクリプト化** | **サイクル3でLeadがB3条件4を甘く評価。数値基準は確率的制御では不十分** | **全ての収束条件をHook化（→ 定性的基準には過剰。定量的基準のみスクリプト化）** |
